...
public class HReplicationServerCommandLine extends ServerCommandLine {
  ...
  private static final String USAGE =
      "Usage: HReplicationServer [-D conf.param=value] start";

  public HReplicationServerCommandLine(Class<? extends HReplicationServer> clazz) {
    this.replicationServerClass = clazz;
  }
  ...
  private int start() throws Exception {
    ...
    try {
      // If 'local', don't start a replication server here. Defer to
      // LocalHBaseCluster. It manages 'local' clusters.
      if (LocalHBaseCluster.isLocal(conf)) {
        LOG.warn("Not starting a distinct replication server because "
            + HConstants.CLUSTER_DISTRIBUTED + " is false");
      } else {
        logProcessInfo(getConf());
        HReplicationServer hrs = HReplicationServer.constructReplicationServer(replicationServerClass, conf);
        hrs.start();
        hrs.join();
        if (hrs.isAborted()) {
